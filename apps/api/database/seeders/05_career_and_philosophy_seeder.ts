...
export default class CareerAndPhilosophySeeder extends BaseSeeder {
  async run() {
    ...
    // 4. Services (Data with SVG Icons & FAQs)
    const services = [
      {
        ...
        faqs: [
          ...
          },
          {
            ...
            answer:
              'Ya, seluruh source code, dokumentasi API, dan instruksi deployment di Docker menjadi milik klien secara 100%.',
            sortOrder: 2,
          },
        ],
      },
      {
        title: 'Backend API Architecture & Optimization',
        ...
        shortDescription:
          'Enterprise-grade REST APIs, authentication security, and high-concurrency database modeling.',
        valueProposition:
          'Engineered for reliability, zero downtime migrations, and sub-100ms response times.',
        deliverables: [
          'RESTful & GraphQL Endpoints',
          'Argon2id & Passkey Biometric Auth',
          'Redis Caching & Queue Worker Setup',
        ...
        faqs: [
          {
            question: 'Apakah backend dirancang untuk tahan terhadap lonjakan trafik?',
            answer:
              'Tentu. Kami menerapkan Redis caching, rate limiting cerdas, async queues, dan connection pooling PostgreSQL.',
            sortOrder: 1,
          },
        ],
      },
      {
        title: 'Automated Telegram Bots & Integrations',
        slug: 'automated-bots-integrations',
        iconSvg: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-sky-400"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg>`,
        shortDescription:
          'Automated Telegram bots, lead dispatchers, and third-party webhook integrations.',
        valueProposition:
          'Automate business workflows and capture real-time customer leads directly to your Telegram chat.',
        deliverables: [
    ...
    for (const srvData of services) {
      ...
      const srv = await Service.updateOrCreate({ slug: serviceFields.slug }, serviceFields)
    ...
    await Philosophy.query().delete()
    for (const phil of philosophies) {
      await Philosophy.create(phil)
    }

    // 6. Areas of Expertise (Keahlian & Spesialisasi)
    const expertises = [
      {
    ...
    // 7. Workflow Steps (Alur Kerja Kolaborasi)
    const workflowSteps = [
      {
        stepNumber: '01',
