...
import { BlogCatalogView } from "@/components/blog/blog-catalog-view";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = `Blog & Tulisan — ${settings.profile.siteName}`;
  const description = `Kumpulan artikel dan catatan seputar teknologi, edukasi, perjalanan karir, tips praktis, dan pengalaman oleh ${settings.profile.ownerName || "Muhammad Ihsan Maulana"}.`;
...
function BlogLoadingFallback() {
  return (
    <div className="space-y-10 animate-pulse">
      <div className="h-32 rounded-3xl bg-card/40 border border-border/60" />
...
export default async function BlogCatalogPage() {
  const [settings, articlesResponse, categories, tags] = await Promise.all([
    getSiteSettings(),
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-3xl mb-12 sm:mb-16">
            <div className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3.5 py-1 text-xs font-semibold text-primary dark:text-emerald-400 mb-4">
              ...
              <span>Blog, Edukasi &amp; Catatan</span>
            </div>

            <h1 className="text-3xl sm:text-5xl font-extrabold font-heading tracking-tight text-foreground leading-[1.15] mb-4">
              Artikel, Catatan &amp; Tips
            </h1>

            <p className="text-sm sm:text-base text-muted-foreground leading-relaxed">
              Kumpulan tulisan seputar teknologi, edukasi, perjalanan karir,
              tips bermanfaat, dan ruang berbagi pengalaman untuk saling
              bertumbuh.
            </p>
          </div>

          {/* Interactive Catalog View with live search & numbered pagination wrapped in Suspense */}
