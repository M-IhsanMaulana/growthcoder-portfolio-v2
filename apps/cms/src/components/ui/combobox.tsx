...
export function Combobox({
  ...
  const sizeClasses = {
    sm: "h-8.5 px-2.5 text-xs rounded-lg",
    default: "h-10 px-3 text-sm rounded-xl",
    lg: "h-11 px-3.5 text-base rounded-xl",
  };
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          disabled={disabled}
          className={cn(
            "w-full justify-between border-input bg-background font-normal text-foreground shadow-xs hover:bg-muted/50 cursor-pointer text-left",
            sizeClasses[size],
        ...
        >
          ...
          <ChevronsUpDown className="ml-2 size-3.5 shrink-0 opacity-50 text-muted-foreground" />
        </Button>
      </PopoverTrigger>
      <PopoverContent
        className={cn(
          "w-[var(--radix-popover-trigger-width)] min-w-[200px] p-0 border border-border bg-popover text-popover-foreground shadow-xl rounded-xl overflow-hidden",
          contentClassName,
        )}
        align="start"
      >
              ...
              {options.map((option) => {
                ...
                return (
                  <CommandItem
                    ...
                    className={cn(
                      "flex items-center justify-between gap-2.5 px-3 py-2.5 cursor-pointer rounded-lg transition-colors",
                      isSelected
